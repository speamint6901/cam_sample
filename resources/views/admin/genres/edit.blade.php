    ...
    <form class="form-horizontal" method="post" action="{{ route('genres.update', $genre->id) }}">
      ...
      <div class="box-body">
        <div class="form-group">
          ...
          <div class="col-sm-10">
            {{ Form::select('category_id', \App\Models\Category::selectbox(), $genre->category_id, ['class' => 'form-control', 'id' => 'category_id']) }}
          </div>
        </div>
        <div class="form-group">
          <label for="name-text" class="col-sm-2 control-label">ジャンル名</label>
          <div class="col-sm-10">
            <input type="text" class="form-control" name="name" id="name-text" value="{{ $genre->name }}" placeholder="ジャンル名">
